...
class ServerlessCloudfrontDistributionCertificate {
    constructor(serverless, options) {
        this.validationChecks = 15;
        this.serverless = serverless;
        this.options = options;
        this.waitForCertificateToBecomeValid = this.waitForCertificateToBecomeValid.bind(this);
        this.hooks = {
            "aws:package:finalize:mergeCustomProviderResources": this.assignCert.bind(this),
        };
    }
    assignCert() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield this.createCert();
            if (!this.cerArn) {
                throw new Error("Something went wrong");
            }
            else {
                yield this.checkAndCreateRoute53Entry();
            }
            this.modifyCloudformation();
            this.validationChecks =
                this.serverless.service.custom.cfdDomain.retries || 15;
            yield this.waitForCertificateToBecomeValid();
        });
    }
    delay(ms) {
        return new Promise((resolve) => setTimeout(resolve, ms));
    }
    waitForCertificateToBecomeValid() {
        return __awaiter(this, void 0, void 0, function* () {
            const cert = yield this.acm
            ...
            if (cert.Certificate.Status === "PENDING_VALIDATION" &&
                ...
                yield this.delay(60000).then(this.waitForCertificateToBecomeValid);
            }
    ...
    checkAndCreateRoute53Entry() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            do {
                ...
                const certificate = yield this.acm
                    .describeCertificate({ CertificateArn: this.cerArn })
                    .promise();
                ...
                validations = certificate.Certificate.DomainValidationOptions.filter(({ ValidationStatus, ValidationMethod, ResourceRecord }) => ValidationStatus === "PENDING_VALIDATION" &&
            ...
            const validationPromises = validations.map((validation) => __awaiter(this, void 0, void 0, function* () {
                this.serverless.cli.log(`Validating certificate`);
                const hostedZone = yield this.findHostedZoneId(validation.ResourceRecord.Name);
                if (!hostedZone) {
                ...
                const params = {
                    ChangeBatch: {
                        ...
                        Comment: "Record created by serverless-cloudfront-distribution-certificate",
                    },
    ...
    findHostedZoneId(domain) {
        return __awaiter(this, void 0, void 0, function* () {
            this.serverless.cli.log(`Getting hosted zone id`);
            const zones = yield this.getHostedZones();
            ...
            const targetHostedZone = zones.filter((hostedZone) => {
                ...
                if (domainNameReverse.length === 1 ||
                    domainNameReverse.length >= hostedZoneNameReverse.length) {
                    return !hostedZoneNameReverse.some((n, i) => n !== domainNameReverse[i]);
                }
                return false;
            })
    ...
    createCert() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const certData = yield this.acm
                .listCertificates({ CertificateStatuses: statuses })
                .promise();
            const domain = this.domains[0];
            const alternativeNames = this.domains.slice(1);
            const certificates = certData.CertificateSummaryList.filter((cer) => cer.DomainName === domain);
            let certificate = null;
            if (certificates.length > 0) {
                const certificateDetailPromises = certificates.map((currentCertificate) => this.acm
                    .describeCertificate({
                    CertificateArn: currentCertificate.CertificateArn,
                })
                    .promise());
                const certificateDetails = yield Promise.all(certificateDetailPromises);
                certificate = certificateDetails.find((certificateDetail) => {
                    const alternativeNamesSet = new Set(certificateDetail.Certificate.SubjectAlternativeNames);
                    this.serverless.cli.log(`Checking: ${certificateDetail.Certificate.DomainName}`);
                    return !alternativeNames.some((aN) => !alternativeNamesSet.has(aN));
                });
            }
            if (certificate) {
                this.serverless.cli.log(`Found existing certificate: ${certificate.Certificate.CertificateArn}`);
                ...
                const cerArn = yield this.acm.requestCertificate(certParams).promise();
    ...
    modifyCloudformation() {
        const template = this.serverless.service.provider
            .compiledCloudFormationTemplate;
        template.Resources[this.cloudFront].Properties.DistributionConfig.ViewerCertificate = {
...
module.exports = ServerlessCloudfrontDistributionCertificate;
